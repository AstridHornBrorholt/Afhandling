...
with hybrid dynamics #cite(label("HybridPaper")). The
...
work #cite(label("HybridPaper")). We start by recalling the
...
intuition. (See #cite(label("HybridPaper")) for details.)
...
(In #cite(label("HybridPaper")), we only allowed a grid of
...
to #cite(label("HybridPaper")) but adapted to work in
...
observed in #cite(label("HybridPaper")), the more
...
In #cite(label("HybridPaper")), stochasticity was treated as
...
converter] #cite(label("HybridPaper")) models a real circuit
...
model #cite(label("HybridPaper")) #cite(label("DBLP:conf/isola/Jaeger0BLJ20")),
...
from #cite(label("HybridPaper")) in the tool
