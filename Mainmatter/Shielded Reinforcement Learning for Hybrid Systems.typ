...
systems @lewis2012optimal @doyle2013feedback. This motivates
the usage of rl, which is a powerful machine-learning method applicable
to systems with complex and stochastic dynamics @BusoniuBTKP18.
...
control @VlachogiannisH04 @NoaeenNGCAABF22. A typical way to
...
punishes unsafe behavior @10.5555/2789272.2886795. This will
...
actions #label("DBLP:conf/tacas/BloemKKW15.") This makes it possible to
...
automaton #label("DBLP:journals/corr/abs-1208-3856.") While HMDPs
...
serious restrictions @HenzingerKPV98. These complexity barriers
...
in the tool #smallcaps[Uppaal Stratego] @stratego. The term
"shielding" was coined by Bloem et
al. #label("DBLP:conf/tacas/BloemKKW15,") who introduced special
...
certificates and adversarial examples @ChengOMB19 @LuoM21 or
Lyapunov functions @HasanbeigAK20. Similarly, Berkenkamp et
al. describe a method to provide a safe policy with high
probability @BerkenkampTS017. Chow et al. consider a relaxed
version of safety based on expected cumulative cost @ChowNDG18.
...
Larsen et al. @10.1007/978-3-030-23703-5_6 used a set-based
...
synthesis @MajumdarOS20 and reachability
analysis @KlischatA20 and is also used in recent work on
learning a safe controller for discrete stochastic systems in a
teacher-learner framework @ZikelicLHC23. Contemporary work by
Badings et al. @badings_robust_2023 also uses a finite
...
state-space partitioning @AbateAPLS07 @ShmarovZ15, which we
also employ in this work, or have a statistical
angle @Bujorianu12. We are not aware of any works that extended
SHSs to HMDPs.
...
(EMDP) #label("DBLP:conf/atva/JaegerJLLST19") @randomwalk is a
...
r0.37 #image\("Graphics/RWExample.svg", width: 100)

<fig:RandomWalk>

==== Example  <ex:random_walk> (Random walk).
<example-random-walk.>
...
$epsilon.alt$ models the uncertainty.$lt.tri$
...
recursive equation:
$ bb(E)_sigma^(cal(M)) lr((s)) eq cases(delim: "{", 0 & upright("if ") s in cal(G), sum_(a in italic("Act")) sigma lr((s)) lr((a)) dot.op integral_(s prime in cal(S)) T lr((s comma a)) lr((s prime)) dot.op #scale(x: 120%, y: 120%)[paren.l] C lr((s comma a comma s prime)) plus bb(E)_sigma^(cal(M)) lr((s prime)) #scale(x: 120%, y: 120%)[paren.r] thin d s prime & upright("if ") s in.not cal(G)) $
...
(SHS) #label("DBLP:journals/corr/abs-1208-3856") #label("DBLP:conf/formats/Larsen12,")
which allows the environment to interleave continuous evolution and
discrete jumps.

#figure([#figure([#image\("Graphics/BBSimple.png", width: 96%)],
    caption: [
      SHA for the bouncing ball.
    ]
  )

  #figure([#image\("Graphics/BallBounceDensity.svg", width: 100%)],
    caption: [
...
(SHA) #label("DBLP:journals/corr/abs-1208-3856,") which we only introduce
...
- $mu lr((lr((p comma v)))) eq delta #scale(x: 120%, y: 120%)[paren.l] lr((v plus sqrt(v^2 plus 2 dot.op 9.81 dot.op p))) slash 9.81 #scale(x: 120%, y: 120%)[paren.r]$
...
#figure([#figure([#image\("Graphics/UnshieldedTrace1.svg", width: 100)],
    caption: [
      Strategy.
    ]
  )

  #figure([#image\("Graphics/UnshieldedTrace2.svg", width: 100)],
    caption: [
...
r0.60 #image\("Graphics/HBB.png", width: 100)
...
finite partitioning (similar to @ZikelicLHC23) of the state
...
$cal(A) eq brace.l mu in cal(I)_gamma^k divides mu sect cal(S) eq.not nothing brace.r$
...
#figure([#figure([#image\("Graphics/SquaresReachabilityBarbaric.svg", width: 100)],
    caption: [
      Scenario where the ball is rising and high enough to be hit.
    ]
  )

  #figure([#image\("Graphics/SquaresReachabilityBounceBarbaric.svg", width: 100)],
    caption: [
      Scenario where the ball is too low to be hit, but bounces off the
      ground.
    ]
  )
...
such that
$ bb(S)_phi eq phi^A sect brace.l mu divides exists a dot.basic med forall mu prime dot.basic med mu arrow.r^a mu prime arrow.r.double.long mu prime in bb(S)_phi brace.r dot.basic $
...
strategy $nu_phi$ obtained from $bb(S)_phi$ @BernetJW02 is given
by
...
decided @Tarski48, which may however require doubly exponential
time @DavenportH88, and worse, when considering nonlinear
dynamics with, e.g., trigonometric functions, the problem becomes
undecidable @Laczkovich03. One can obtain a conservative answer
via over-approximate reachability analysis @DFPP18; in
...
are simulated according to the SHS $H$ underlying the given HMDP $H M$.

r0.5

$R eq nothing$ select $s_i prime tilde.op N lr((s_i comma a))$ simulate
$H$ from $s_i prime$ for $P$ time units let $s_i prime.double$ be the
resulting state add $lr([s_i prime.double])_A$ to $R$
...
damping.
@fig:SquaresReachabilityBarbaricGroup
...
#figure([#image\("Graphics/RWShield.svg", width: 100)
  #image\("Graphics/BBShield.svg", width: 100)],
  caption: [
    Synthesized strategies for random walk (left) and bouncing ball
    (right).
  ]
)
<fig:ShieldsRWBB>
...
#figure([#image\("Graphics/StrategoMyPreShielding.svg", width: 100)],
  caption: [
...
We use #smallcaps[Uppaal Stratego] @stratego to train a shielded
...
- #emph[Cruise
  control] @larsen2015cruisecontrol", "10.1007/978-3-030-23703-5_6 #label("DBLP:conf/qest/AshokKLCTW19:")
...
- #emph[DC-DC converter] @dcdcconverter: This industrial DC-DC
...
- #emph[Oil pump] @hydac: In this industrial case, flow of oil
...
#figure([#figure([#image\("Graphics/CCShield.svg", width: 100)],
    caption: [
      Cruise control ($n eq 4$, $gamma eq 0.5$) when the car’s velocity
      is $0 m slash s$
    ]
  )
  <fig:CCShield>
  ...
  #figure([#image\("Graphics/DCShield.svg", width: 100)],
    caption: [
  ...
  #figure([#image\("Graphics/OPShieldOn.svg", width: 100)],
    caption: [
...
Stratego] @stratego for learning and statistical model checking.
The experiments are available online @REP.
...
#figure([#image\("Graphics/BarbaricAccuracyN.svg", width: 1100)
  #image\("Graphics/BarbaricAccuracyG.svg", width: 1100)],
  caption: [
...
#smallcaps[JuliaReach] @JuliaReach. Given a set of initial
...
reachability algorithm from @GuernicG09. This algorithm uses
...
#align(center)[#table(
  ...
  [$n eq 4$],
  [2m 14s],
  [$lr([99.9999 percent semi 100 percent])$],
  [0.02],
  [],
  [$n eq 8$],
  [4m 02s],
  ...
  [$lr([99.9999 percent semi 100 percent])$],
  [0.01],
  [#smallcaps[JuliaReach]],
  [time step $0.002$],
  [24h 30m],
  [considers $s_0$ unsafe],
  [0.01],
  [],
  [time step $0.001$],
  [41h 05m],
...
r0.41
#image\("Graphics/DifferenceRigorousBarbaric.svg", width: 100)
...
#figure([#figure([#image\("Graphics/PreShieldedTrace.svg", width: 100)],
    caption: [
      Pre-shield.
    ]
  )

  #figure([#image\("Graphics/PostShieldedTrace.svg", width: 100)],
    caption: [
      Post-shield.
    ]
  )
...
@tab:synthesis reports the same data as
...
#align(center)[#table(
  ...
  inset: 6pt,
  [#strong[$gamma$]], [#strong[$n$]], [#strong[Time]],
  [#strong[Probability safe]],
  [1],
  [2],
  [1m 50s],
  [Considers $s_0$ unsafe],
  ...
  [2],
  [13m 16s],
  [$lr([99.9995 percent semi 100 percent])$],
  [0.5],
  [3],
  [23m 03s],
  [$lr([99.9995 percent semi 100 percent])$],
...
#align(center, [DC-DC boost converter. $gamma eq 0.05$ corresponds to
...
#figure([#figure([#image\("Graphics/RWShieldingResults.svg", width: .95100)],
    caption: [
  ...
  #figure([#figure([#image\("Graphics/RWShieldingDeaths.svg", width: .95100)],
      caption: [
        Safety violations for unshielded agents
      ]
    )
    <fig:RWShieldingDeaths>

    #figure([#image\("Graphics/RWShieldingInterventions.svg", width: .95100)],
      caption: [
        Interventions for post-shielded agents.
      ]
    )
...
#figure([#figure([#image\("Graphics/BBShieldingResults.svg", width: .95100)],
    caption: [
      Average #emph[hit] actions per 120s.
    ]
  )
  <fig:BBShieldingResults>

  #figure([#figure([#image\("Graphics/BBShieldingDeaths.svg", width: .95100)],
      caption: [
        Safety violations for unshielded agents.
      ]
    )
    ...
    #figure([#image\("Graphics/BBShieldingInterventions.svg", width: .95100)],
      caption: [
        Interventions for post-shielded agents.
      ]
    )
...
#figure([#figure([#image\("Graphics/CCShieldingResults.svg", width: .95100)],
    caption: [
      Accumulated distance per 120s.
    ]
  )
  <fig:CCShieldingResults>

  #figure([#figure([#image\("Graphics/CCShieldingDeaths.svg", width: .95100)],
      caption: [
        Safety violations for unshielded agents.
      ]
    )
    ...
    #figure([#image\("Graphics/CCShieldingInterventions.svg", width: .95100)],
      caption: [
        Interventions for post-shielded agents.
      ]
    )
...
#figure([#figure([#image\("Graphics/DCShieldingResults.svg", width: .95100)],
    caption: [
      Accumulated error plus number of switches per 120s.
    ]
  )
  <fig:DCShieldingResults>

  #figure([#figure([#image\("Graphics/DCShieldingDeaths.svg", width: .95100)],
      caption: [
        Safety violations for unshielded agents.
      ]
    )
    ...
    #figure([#image\("Graphics/DCShieldingInterventions.svg", width: .95100)],
      caption: [
        Interventions for post-shielded agents.
      ]
    )
...
#figure([#figure([#image\("Graphics/OPShieldingResults.svg", width: .95100)],
    caption: [
      Accumulated oil volume per 120s.
    ]
  )
  <fig:OPShieldingResults>

  #figure([#figure([#image\("Graphics/OPShieldingDeaths.svg", width: .95100)],
      caption: [
        Safety violations for unshielded agents.
      ]
    )
    ...
    #figure([#image\("Graphics/OPShieldingInterventions.svg", width: .95100)],
      caption: [
        Interventions for post-shielded agents.
      ]
    )
...
#align(center)[#table(
  ...
  inset: 6pt,
  [#strong[Configuration]], [#strong[Cost]], [],
  [#strong[Interventions]], [],
  [Baseline with uniform random choice],
  [11371],
  [],
  [13.50],
  [],
...
at the preference of the shielded agent (the so-called
Q-value @Watkins89).
